...
def local_density_p2r(adatafilt):
## calculate intracellular correlations using points2regions.
#In summary we (1) binnarize the expression of a cell and compute:
#   - minimum correlation between two bins
#  -  mean correlation beween all intracellular bins
    ...
    for a,g in tqdm(adatafilt.uns['spots'].groupby('cell_id')):
        xy = g[['x_location', 'y_location']].to_numpy()
        genes = g['feature_name'].to_numpy()
        sigma = 3.0#3.0
        bin_width = 1.0
        min_genes_per_bin = 0
        n_clusters=2
        results = points2regions_adapted(xy, genes, sigma, n_clusters, bin_width, min_genes_per_bin, groupids=None, return_anndata=True)
        #counts=results['id']['features'].sum(axis=1)
        #densdif=np.max(counts)-np.min(counts)
        #allds.append(densdif)
        matr=pd.DataFrame(results['id']['features'].todense())
        correl=pd.DataFrame(matr.transpose()).corr()
        mincor.append(np.min(correl))
        meancor.append(np.mean(correl))
        cell_ids.append(a)
    id2min=dict(zip(cell_ids,mincor))
    id2mean=dict(zip(cell_ids,meancor))
    adatafilt.obs['intracellular_min_correlation']=adatafilt.obs['cell_id'].map(id2min)
    adatafilt.obs['intracellular_mean_correlation']=adatafilt.obs['cell_id'].map(id2mean)
    return adatafilt
