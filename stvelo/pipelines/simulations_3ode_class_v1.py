...
class Simulation3ODE:
    ...
    def simulation(self):
        ...
        if self.generate_switch_times:
            switches = self.switch_times( n_vars = n_vars)
            # switches.sort()
            switches = self.cycle(switches,n_vars)
        else:
            switches = self.cycle([0.2, 0.4, 0.7, 0.1], n_vars)
        ...
        layer_spliced_unspliced = {"unspliced": U, "spliced": Sn + Sc, "spliced_nuc":Sn ,"spliced_cyt":Sc, "true_velocity": true_velocity_s_u}
        layer_nuc_cyto = {"unspliced": U + Sn, "spliced": Sc, "spliced_nuc":Sn,'spliced_cyt':Sc, "true_velocity": true_velocity_n_c }
